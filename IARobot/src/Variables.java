/**
 * Compilation of Variables
 */
public class Variables {
	public static final String GATE1_BRICK_NAME = "Tore";
	public static final String GATE2_BRICK_NAME = "Tore2";
	public static final String TURNTABLE_BRICK_NAME = "Turntable";
	public static final int COMMAND_CLOSE_CONNECTION = 1337;
	
	public static final int COLOR_RED = 0;
	public static final int COLOR_BLUE = 1;
	public static final int COLOR_ORANGE = 2;
	
	public static final int ROTATION_CORRECT = 180;
	public static final int ROTATION_WRONG = 90;
	public static final int SYMBOLS = 3;
}
